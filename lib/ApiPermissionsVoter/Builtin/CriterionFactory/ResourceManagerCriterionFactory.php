...
class ResourceManagerCriterionFactory extends CriterionFactory {

	/** @var array<string, array<int, ResourceManagerCriterion>> */
	private array $cache = [];
	...
	public function build(Principal $principal, Resource $resource): ResourceManagerCriterion {
		...
		return $this->cache[$principalKey][$resource->getId()] = new ResourceManagerCriterion(
			$this->l10n,
			$principal,
			$resource,
			$this->scratchpad,
	...
	public function buildFromId(Principal $principal, int $resourceId): ResourceManagerCriterion {
		$principalKey = $principal->getKey();
