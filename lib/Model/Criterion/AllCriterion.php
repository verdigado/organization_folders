...
class AllCriterion extends CompositeCriterion {
	...
	public function evaluate(bool $allReasons = false, array $criterionTypeBlocklist = []): CriterionResult {
		...
		if($allReasons) {
			...
			// return after first unsatisfied operand
			$satisfiedReasons = [];

			foreach($this->operands as $operand) {
				$evalResult = $operand->evaluate($allReasons, $criterionTypeBlocklist);

				if($evalResult instanceof CriterionUnsatisfied) {
					$result = $evalResult;
					...
					return $result;
				} else if ($evalResult instanceof CriterionSatisfied) {
